...
Genus_root <- ggplot(data = filter(Mines.df, Type == "Root"), aes(Site, Abundance, fill = Genus)) +
  geom_bar(stat = "identity", position = position_fill()) + coord_flip() +
    ...
    facet_grid(~ Type, drop = TRUE) +
   theme(text = element_text(size = 15)) + theme_gray()
plot_grid(Genus_root, Genus_soil, align = "h", rel_widths = c(1,1.1))
...
Class_root <-ggplot(data = filter(Mines.df, Type == "Root"), aes(Site, Abundance, fill = Class)) +
  ...
  facet_grid(~ Type, drop = TRUE) +
  theme(text = element_text(size = 15)) + theme_gray()
plot_grid(Class_root, Class_soil, align = "h", rel_widths = c(1,1.1))
...
Genus_soil <- ggplot(data = filter(Mines.df, Type == "Soil" & !is.na(Genus))) +
  geom_bar(aes(Site, Abundance, fill = Genus), stat = "identity", position = position_fill()) + coord_flip() +
    scale_fill_manual(values = c ("#f0f0f0", "#02818a", "#3690c0", "#8c6bb1", "#fcc5c0", "#bd0026", "#fc4e2a","#feb24c", "#ffffcc", "#e5f5e0", "#a1d99b", "#41ab5d", "#006d2c", "#deebf7", "#9ecae1", "#4292c6", "#08519c")) +
    guides(fill = guide_legend(reverse = TRUE, ncol = 1, keyheight = 0.8)) +
    facet_grid(~ Type, drop = TRUE) + 
    theme(text = element_text(size = 15)) + theme_gray()
...
top10.BGM <- TopNOTUs(BGM_mine, 10)
...
BGM.genus <- summarise (test, relative_abundance = (sum(Abundance)/sum(sum))*100)
...
HKM.genus <- summarise (test, relative_abundance = (sum(Abundance)/sum(sum))*100)
...
SH.genus <- summarise (test, relative_abundance = (sum(Abundance)/sum(sum))*100)
...
RGM.genus <- summarise (test, relative_abundance = (sum(Abundance)/sum(sum))*100)
